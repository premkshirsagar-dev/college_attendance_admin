...
const TeacherForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchTeacher = async () => {
      try {
        ...
        setMessage({ type: "error", text: "Failed to load teacher." });
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      <Navbar title={isEdit ? "Edit Teacher" : "Add Teacher"} />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
          ...
          {loading ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              >
                {saving ? "Loading..." : isEdit ? "Save Changes" : "Add Teacher"}
              </button>
            </form>
          )}
        </div>

        <Link to="/teachers" className="text-brand-600 font-medium text-sm mt-6 inline-block">
